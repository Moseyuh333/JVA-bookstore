        ...
        <div class="bg-white rounded-lg shadow-lg p-8 max-w-4xl mx-auto">
          <div class="flex flex-col md:flex-row gap-8">
            <img src="<c:out value='${bookImage != null && bookImage ne "" ? bookImage : "
              http://static.photos/books/640x480/1"}' />"
            alt="${bookTitle}"
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center gap-3 mb-4">
                <c:choose>
                  <c:when test="${not empty bookRatingInt}">
                    <% Object ratingObj=request.getAttribute("bookRatingInt"); double rating=0.0; if (ratingObj !=null)
                      { try { rating=Double.parseDouble(ratingObj.toString()); } catch (Exception ignored) {} } %>
                      <span
                        class="inline-flex items-center gap-1 bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-sm">
                        ★ <%= String.format("%.1f", rating) %>/5
                      </span>
                  </c:when>
                  <c:otherwise>
                    <span
                      class="inline-flex items-center gap-1 bg-gray-100 text-gray-500 px-3 py-1 rounded-full text-sm">
                      ★ 0.0/5
                    </span>
                  </c:otherwise>
                </c:choose>
                <span class="text-sm">
                  <c:choose>
                    <c:when test="${bookInStock}"><span class="text-green-600 font-medium">${bookStockText}</span>
                    </c:when>
