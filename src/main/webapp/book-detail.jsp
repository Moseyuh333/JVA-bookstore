              ...
              <c:choose>
                <c:when test="${not empty reviews}">
                  <div class="bg-[#1b1b1b] mt-10 p-8 rounded-lg shadow-md">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-8">
                      <div class="text-center md:text-left md:w-1/4">
                        ...
                        <div class="flex justify-center md:justify-start mt-1 mb-2">
                          <c:forEach var="i" begin="1" end="5">
                            <svg xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div class="py-5 flex flex-col gap-2">
                          ...
                          <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                              <div
                                class="bg-[#333] text-gray-400 rounded-full h-8 w-8 flex items-center justify-center font-bold">
                                ${fn:toUpperCase(fn:substring(r.authorName,0,1))}
                              </div>
                              <div>
                                <p class="text-gray-200 font-semibold">${r.authorName}</p>
                                <p class="text-gray-500 text-xs">Đã mua hàng</p>
                              </div>
                            </div>
                  ...
                  -3.39-2.463c-.783-.57-.381-1.81.588-1.81h4.18l1.287-3.975z" />
                            ...
                            <span class="text-sm text-amber-400 ml-2">Cực kì hài lòng</span>
                          </div>
                  ...
                  <div class="bg-[#1b1b1b] mt-10 p-8 rounded-lg shadow-md">
                    ...
                    <div class="space-y-6">
                      <c:forEach var="rev" items="${fn:split(bookReviewsRaw, '\\|')}">
                        <!-- Tách tên, sao và nội dung -->
                        <c:set var="name" value="${fn:substringBefore(rev, '(')}" />
                        <c:set var="afterParen" value="${fn:substringAfter(rev, '(')}" />
                        <c:set var="stars" value="${fn:substringBefore(afterParen, ')')}" />
                        <c:set var="comment" value="${fn:substringAfter(rev, ')')}" />

                        <div class="bg-[#222] p-4 rounded-lg">
                          <!-- Header -->
                          <div class="flex justify-between items-center mb-2">
                            <span class="text-amber-400 font-semibold">${fn:trim(name)}</span>
                            <div class="flex items-center">
                              <c:forEach var="i" begin="1" end="5">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                  class="h-4 w-4 ${i <= fn:substringBefore(stars, '⭐') ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600 fill-gray-600'}"
                                  viewBox="0 0 20 20" fill="currentColor">
                    ...
                    -3.39-2.463c-.783-.57-.381-1.81.588-1.81h4.18l1.287-3.975z" />
                          ...
                          <!-- Nội dung bình luận -->
                          <p class="text-gray-300 leading-relaxed">${fn:trim(comment)}</p>
                        </div>
                      </c:forEach>
                    </div>
                  </div>
